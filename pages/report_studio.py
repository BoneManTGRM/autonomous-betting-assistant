...
TRUTH_CONTRACT_VERSION = safe_text(getattr(magazine_book_export, "_ABA_SALE_READY_TRUTH_CONTRACT_VERSION", "truth_contract_unknown"))
ACTIVE_EXPORT_VERSION = f"{magazine_book_export.MAGAZINE_STYLE_VERSION}:{NO_MARKET_EXPORT_VERSION}:{ENRICHMENT_VERSION}:{TRUTH_CONTRACT_VERSION}"
...
def cached_render_full_pick_magazine_page_png(row_items: tuple[tuple[str, str], ...], background_bytes: bytes | None, report_name: str, page_number: int, total_pages: int, language: str, style_version: str, no_market_version: str, enrichment_version: str, truth_contract_version: str) -> bytes:
    rowd = with_report_language(dict(row_items), language)
    rowd["_magazine_style_version"] = f"{style_version}:{no_market_version}:{enrichment_version}:{truth_contract_version}"
...
with tabs[6]:
    ...
    book_cache_key = f"report_studio_full_book_export_cache_{LANG}_{brand_cache}_{magazine_book_export.MAGAZINE_STYLE_VERSION}_{NO_MARKET_EXPORT_VERSION}_{ENRICHMENT_VERSION}_{TRUTH_CONTRACT_VERSION}"
    if st.button(t("build_book"), key=f"report_studio_prepare_full_book_{LANG}_{brand_cache}_{magazine_book_export.MAGAZINE_STYLE_VERSION}_{NO_MARKET_EXPORT_VERSION}_{ENRICHMENT_VERSION}_{TRUTH_CONTRACT_VERSION}"):
    ...
    if cards_as_rows:
        ...
        selected_idx = st.selectbox(t("select_page"), range(len(pick_options)), format_func=lambda i: pick_options[i], key=f"report_studio_selected_full_page_{LANG}_{brand_cache}_{magazine_book_export.MAGAZINE_STYLE_VERSION}_{NO_MARKET_EXPORT_VERSION}_{ENRICHMENT_VERSION}_{TRUTH_CONTRACT_VERSION}")
        selected_row = cards_as_rows[int(selected_idx)]
        selected_png = cached_render_full_pick_magazine_page_png(serializable_row(selected_row), background_bytes, magazine_report_name, int(selected_idx) + 1, len(cards_as_rows), LANG, magazine_book_export.MAGAZINE_STYLE_VERSION, NO_MARKET_EXPORT_VERSION, ENRICHMENT_VERSION, TRUTH_CONTRACT_VERSION)
