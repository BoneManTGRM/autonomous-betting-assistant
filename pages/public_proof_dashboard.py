...
TEXT = {
    'en': {
        ...
        'test_window': 'Test Window ID', 'active_test_window': 'Active test ledger',
        'use_db': 'Use full saved database', 'use_session': 'Use current synced dashboard rows', 'use_demo': 'Show demo ledger if no real ledger exists',
        'upload_ledger': 'Upload locked ledger or historical tracker CSV', 'upload_results': 'Upload finished results CSV for auto-grading',
        'save_db': 'Save current dashboard ledger to this test ledger', 'apply_results': 'Apply result updates', 'source': 'Source',
        'active_source': 'Active proof source', 'rows': 'Locked', 'resolved': 'Resolved', 'record': 'Record', 'hit_rate': 'Hit rate',
        'roi': 'ROI', 'units': 'Units', 'pending': 'Pending', 'clv': 'Avg CLV', 'proof_quality': 'Proof quality',
        'valid': 'Valid proof rows', 'ignored_nonproof': 'Ignored raw/non-proof rows', 'filtered': 'Filtered rows', 'filters': 'Dashboard filters',
        'sport_filter': 'Sport filter', 'market_filter': 'Market filter', 'status_filter': 'Result status filter', 'table': 'Public ledger table',
        'dashboard': 'Breakdowns', 'audit': 'Proof audit', 'cards': 'Report cards', 'brand': 'Brand name', 'card_title': 'Card title',
        'markdown_card': 'Markdown card', 'html_card': 'HTML card', 'daily_report': 'Daily report', 'download_public': 'Download public proof CSV',
        'download_private': 'Download private audit CSV', 'download_audit': 'Download proof audit CSV', 'download_markdown': 'Download Markdown card',
        'download_html': 'Download HTML card', 'download_report': 'Download daily report TXT', 'download_tracker': 'Download normalized tracker CSV',
        ...
        'updated': 'Result update summary', 'tracker_warning': 'This file can be reviewed for record tracking, but it is not official proof because it lacks proof_id and locked_at_utc.',
    },
    'es': {
        ...
        'test_window': 'ID de ventana', 'active_test_window': 'Ledger activo', 'use_db': 'Usar base completa', 'use_session': 'Usar filas sincronizadas actuales',
        'use_demo': 'Mostrar demo', 'upload_ledger': 'Subir CSV', 'upload_results': 'Subir resultados', 'save_db': 'Guardar ledger actual', 'apply_results': 'Aplicar resultados',
        'source': 'Fuente', 'active_source': 'Fuente activa', 'rows': 'Bloqueados', 'resolved': 'Resueltos', 'record': 'Récord', 'hit_rate': 'Acierto', 'roi': 'ROI',
        'units': 'Unidades', 'pending': 'Pendientes', 'clv': 'CLV prom.', 'proof_quality': 'Calidad', 'valid': 'Filas válidas', 'ignored_nonproof': 'Ignoradas/no-prueba',
        'filtered': 'Filas filtradas', 'filters': 'Filtros', 'sport_filter': 'Deporte', 'market_filter': 'Mercado', 'status_filter': 'Estado', 'table': 'Tabla pública',
        'dashboard': 'Desgloses', 'audit': 'Auditoría', 'cards': 'Tarjetas', 'brand': 'Marca', 'card_title': 'Título', 'markdown_card': 'Markdown', 'html_card': 'HTML',
        ...
        'no_rows': 'No hay filas actuales.', 'updated': 'Actualización', 'tracker_warning': 'Este archivo no es prueba oficial porque no tiene proof_id y locked_at_utc.',
    },
}
...
def _has_current_identity(frame: pd.DataFrame) -> bool:
    ...
    for col in IDENTITY_COLUMNS:
        if col in locked.columns and locked[col].map(safe_text).ne('').any():
            return True
    return False
...
def load_saved_session_rows(workspace_id: str) -> tuple[str, pd.DataFrame]:
    ...
    if skipped:
        st.warning('Ignored stale synced dashboard rows with no source_file/active_list_id. Run Pro Predictor Volume again to create a current active proof source, or enable full saved database intentionally.')
    return '', pd.DataFrame()
...
def _unique_text(frame: pd.DataFrame, col: str) -> str:
...
def active_source_summary(frame: pd.DataFrame, source: str, workspace_id: str) -> dict[str, str | int]:
    ...
    return {
        ...
        'source_file': _unique_text(frame, 'source_file'), 'active_list_id': _unique_text(frame, 'active_list_id'), 'ledger_batch_id': _unique_text(frame, 'ledger_batch_id'),
        'rows': int(len(frame)), 'wins': int(status.eq('win').sum()), 'losses': int(status.eq('loss').sum()),
        ...
        'locked_at_min': '' if locked.dropna().empty else locked.min().isoformat(), 'locked_at_max': '' if locked.dropna().empty else locked.max().isoformat(),
        'event_start_min': '' if starts.dropna().empty else starts.min().isoformat(), 'event_start_max': '' if starts.dropna().empty else starts.max().isoformat(),
    }
...
def show_source_summary(frame: pd.DataFrame, source: str, workspace_id: str) -> None:
    ...
    c = st.columns(5)
    c[0].metric('Rows', info.get('rows', 0)); c[1].metric('Wins', info.get('wins', 0)); c[2].metric('Losses', info.get('losses', 0)); c[3].metric('Voids', info.get('voids', 0)); c[4].metric('Pending', info.get('pending', 0))
...
def filter_dashboard(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    with st.expander(t('filters'), expanded=False):
        filtered = frame.copy()
        sport_options = sorted([str(v) for v in filtered.get('sport', pd.Series(dtype=str)).dropna().unique() if str(v).strip()])
        market_options = sorted([str(v) for v in filtered.get('market_type', pd.Series(dtype=str)).dropna().unique() if str(v).strip()])
        status_options = sorted([str(v) for v in filtered.get('result_status', pd.Series(dtype=str)).dropna().unique() if str(v).strip()])
...
def tracker_metrics(frame: pd.DataFrame) -> dict[str, int | float | None]:
    ...
    wins = int(status.eq('win').sum()); losses = int(status.eq('loss').sum()); resolved = wins + losses
...
def show_tracker_mode(frame: pd.DataFrame) -> None:
    normalized = normalize_frame(frame)
    if normalized.empty:
        st.warning(t('no_rows')); st.stop()
    metrics = tracker_metrics(normalized)
    st.warning(t('tracker_warning'))
    cols = st.columns(6)
    cols[0].metric('Rows', metrics['rows']); cols[1].metric(t('resolved'), metrics['resolved']); cols[2].metric(t('record'), f"{metrics['wins']}-{metrics['losses']}"); cols[3].metric(t('hit_rate'), pct(metrics['hit_rate'])); cols[4].metric(t('pending'), metrics['pending']); cols[5].metric(t('proof_quality'), '0/100')
...
with st.expander(t('active_test_window'), expanded=True):
    ...
    use_session = st.checkbox(t('use_session'), value=True, help='ON by default for current synced rows only. Stale rows without source identity are ignored.')
...
if results_upload is not None and not ledger.empty:
    try:
        ...
        if st.button(t('apply_results'), type='primary', use_container_width=True):
            ...
            st.session_state['odds_lock_pro_locked_rows'] = records; st.session_state['public_proof_dashboard_refresh_rows'] = records
            save_held_rows('odds_lock_pro_locked_rows', records, workspace_id); save_held_rows('public_proof_dashboard_refresh_rows', records, workspace_id)
...
raw_cols[0].metric(t('valid'), len(ledger)); raw_cols[1].metric(t('ignored_nonproof'), max(0, raw_count - len(ledger))); raw_cols[2].metric(t('proof_quality'), f"{audit_summary['proof_quality_score']}/100")
cols = st.columns(9)
cols[0].metric(t('rows'), metrics['locked_picks']); cols[1].metric(t('resolved'), metrics['resolved_picks']); cols[2].metric(t('record'), f"{metrics['wins']}-{metrics['losses']}"); cols[3].metric(t('hit_rate'), pct(metrics['hit_rate'])); cols[4].metric(t('roi'), pct(metrics['roi'])); cols[5].metric(t('units'), metrics['profit_units']); cols[6].metric(t('pending'), metrics['pending_picks']); cols[7].metric(t('clv'), pct(metrics.get('avg_clv_percent'), 2)); cols[8].metric('Beat close', pct(metrics.get('beat_close_rate')))
...
with tabs[3]:
    ...
    st.text_area(t('markdown_card'), value=markdown, height=240); st.download_button(t('download_markdown'), markdown, file_name=f'proof_dashboard_card_{workspace_id}.md', mime='text/markdown')
    st.markdown(html, unsafe_allow_html=True); st.text_area(t('html_card'), value=html, height=280); st.download_button(t('download_html'), html, file_name=f'proof_dashboard_card_{workspace_id}.html', mime='text/html')
    st.text_area(t('daily_report'), value=report, height=340); st.download_button(t('download_report'), report, file_name=f'daily_locked_report_{workspace_id}.txt', mime='text/plain')
