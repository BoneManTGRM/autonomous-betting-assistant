...
APP_VERSION = 'pro-predictor-v24-balldontlie-api-registry'
...
def api_coverage_fields(api_context: dict[str, Any], *, odds: bool, sports: bool, weather: bool, api_football: bool = False, perplexity: bool = False, newsapi: bool = False, balldontlie: bool = False) -> dict[str, Any]:
    configured, used = [], []
    if odds:
    ...
    if newsapi:
        ...
        if str(api_context.get('newsapi_source_used', '')).lower() == 'yes':
            used.append('newsapi')
    if balldontlie:
        configured.append('balldontlie')
        if str(api_context.get('balldontlie_source_used', '')).lower() == 'yes':
            used.append('balldontlie')
...
def build_rows(events: list[Any], sport: Any, *, context_builder: LiveAPIContextBuilder, odds_key: str, sports_key: str, weather_key: str, team_filter: str, latest_event_date: date, min_books: int, api_football_key: str = '', perplexity_key: str = '', newsapi_key: str = '', balldontlie_key: str = '') -> list[dict[str, Any]]:
    ...
    for event in events:
        ...
        for outcome in getattr(event, 'outcomes', []) or []:
            ...
            api_context.update(api_coverage_fields(api_context, odds=bool(odds_key), sports=bool(sports_key), weather=bool(weather_key), api_football=bool(api_football_key), perplexity=bool(perplexity_key), newsapi=bool(newsapi_key), balldontlie=bool(balldontlie_key)))
...
saved_balldontlie = get_secret('BALLDONTLIE_API_KEY')
...
api4, api5, api6, api7 = st.columns(4)
...
balldontlie_key = api7.text_input("Ball Don't Lie key", type='password', placeholder='Loaded from secrets' if saved_balldontlie else '').strip() or saved_balldontlie
...
s7.metric("Ball Don't Lie", t('enabled') if balldontlie_key else t('missing'))
