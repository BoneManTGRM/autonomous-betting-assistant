...
def _team(r): return _items(r,((('home_team_snapshot','team_a_snapshot','sports_context_summary'),'Team snapshot','SportsDataIO'),(('away_team_snapshot','team_b_snapshot'),'Team snapshot','SportsDataIO'),(('api_football_context_status',),'Football context','API-Football'),(('newsapi_context_summary',),'News context','NewsAPI')),_src('Agent Model','Context unavailable from current feed'),6)
def _players(r): return _items(r,((('injury_report','injuries','lineup_status','injury_source_reason'),'Roster note','SportsDataIO'),(('sportsdataio_injuries_status',),'Roster status','SportsDataIO'),(('key_players','players','participant_notes'),'Participants','Agent Model')),_src('SportsDataIO','API context not available'),5)
...
def render_full_pick_magazine_page(pick:Any,background_image:Any=None,report_name:str|None=None,page_number:int=1,total_pages:int=1)->Image.Image:
    ...
    _section(d,18,660,338,286,'WHY WE PICKED IT',_why(pick),ca,5,18); _section(d,18,970,338,248,'PRO EVIDENCE',_evidence(pick),cb,5,17); _section(d,382,660,680,372,'TEAM SNAPSHOTS',_team(pick),cb,6,18); _section(d,382,1048,680,170,'PLAYER / INJURY NOTES',_players(pick),cb,5,17); _section(d,18,1238,338,170,'RISK DESK',_risk(pick),ca,4,16); _section(d,372,1238,338,170,'MATCHUP NOTES',_match(pick),cb,4,16); _section(d,726,1238,336,170,'CHAIN NOTES',_chain(pick),cb,4,16)
