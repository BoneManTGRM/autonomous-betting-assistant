...
CSS = '''
<style>
/* Hide Streamlit's automatic multipage navigation. The app renders a curated list below. */
...
def _page_label(language: str, english: str, spanish: str) -> str:
    return spanish if language == 'Español' else english


def render_curated_sidebar(st: Any, language: object = 'English') -> None:
    """Render the stable sidebar below the page's own language selector.

    This function intentionally does not use a persistent session_state guard.
    Streamlit preserves session_state across page switches; a guard caused the
    sidebar to disappear after navigating to a different page.
    """
    language = _normal_language(language)
    ...
    with st.sidebar:
        ...
        for english, spanish, path in PAGES:
            label = _page_label(language, english, spanish)
            try:
...
def install_sidebar_tools() -> None:
    try:
        ...
        return
    if getattr(st, '_ara_curated_sidebar_patch_v6', False):
        return
    st._ara_curated_sidebar_patch_v6 = True
    ...
    def language_radio(label: Any, options: Any, *args: Any, **kwargs: Any) -> Any:
        opts = list(options)
        requested_key = kwargs.get('key')
        current = st.session_state.get(requested_key or 'global_language', st.session_state.get('global_language', 'English'))
        current = _normal_language(current)
        index = opts.index(current) if current in opts else 0
        # Drop selectbox-only args and force the compact radio control the app used before.
    ...
    def sidebar_selectbox(label: Any, options: Any, *args: Any, **kwargs: Any) -> Any:
        if _is_language_widget(label, options):
            return language_radio(label, options, *args, **kwargs)
