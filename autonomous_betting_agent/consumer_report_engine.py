...
def _market_label(market: str, language: str) -> str:
    ...
    en, es = labels.get(str(market or "").lower(), (str(market or "").title() or "Market", str(market or "").title() or "Mercado"))
...
def generate_modeled_parlays(anchor: dict[str, Any], legs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Compatibility shim: guessed-correlation parlays are intentionally disabled."""
    del anchor, legs
    return []
