...
def check_row(check_id: str, title: str, status: str, details: str = "", evidence_id: str = "", expected: Any = "", actual: Any = "") -> dict[str, Any]:
    return {"check_id": check_id, "title": title, "status": status, "details": details, "evidence_id": evidence_id, "expected": expected, "actual": actual}
...
def evidence_summary(evidence_id: str, report: Mapping[str, Any]) -> dict[str, Any]:
    status = report_status(report, ("recovery_status", "restart_status", "audit_status", "system_status", "dashboard_status", "checklist_status", "readiness_status", "bridge_status"))
    return {
...
def status_checks(evidence: Mapping[str, Mapping[str, Any]]) -> list[dict[str, Any]]:
    checks = []
    acceptable = {"CANONICAL RECOVERY SAFE", "RESTART SAFE", "READ ONLY SAFE", "CANONICAL WIRING READY", "DASHBOARD READY", "READY TO REVIEW", "ACTION REQUIRED", "REVIEW REQUIRED"}
...
def acceptance_checks(evidence: Mapping[str, Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        check_row("accept_session_recovery", "Session-empty/disk recovery represented", PASS if canonical.get("resolved_store_name") or canonical.get("recovery_checks") else FAIL, evidence_id="canonical_recovery"),
        check_row("accept_save_reload", "Save/reload verification represented", PASS if canonical.get("save_reload_verification") or restart.get("restart_checks") else FAIL, evidence_id="restart_regression"),
        check_row("accept_dedupe", "Duplicate proof-id handling represented", PASS if "duplicate" in json.dumps(_safe(canonical)).lower() or "duplicate" in json.dumps(_safe(readonly)).lower() else FAIL, evidence_id="canonical_recovery"),
        check_row("accept_workspace", "Workspace mismatch diagnostics represented", PASS if "workspace" in json.dumps(_safe(canonical)).lower() or "workspace" in json.dumps(_safe(readonly)).lower() else FAIL, evidence_id="readonly_audit"),
        check_row("accept_real_page_wiring", "Real page wiring audit represented", PASS if wiring.get("page_results") is not None or wiring.get("system_checks") is not None else FAIL, evidence_id="page_wiring"),
        check_row("accept_dashboard_refresh", "Dashboard refresh package represented", PASS if dashboard.get("dashboard_cards") or dashboard.get("event_breakdown") or dashboard.get("dashboard_rows") is not None else WARN, evidence_id="dashboard_refresh"),
        check_row("accept_operator_review", "Operator review gate represented", PASS if checklist.get("checklist_rows") is not None or checklist.get("readiness_status") or checklist.get("review_checks") is not None else WARN, evidence_id="local_review"),
    ]
...
def summarize_checks(checks: Sequence[Mapping[str, Any]], operator_acknowledged: bool) -> dict[str, Any]:
    ...
    return {"closeout_status": status, "issue_21_recommendation": recommendation, "pass_count": pass_count, "warn_count": warn_count, "fail_count": fail_count, "operator_acknowledged": operator_acknowledged}
...
def build_proof_hardening_closeout(workspace_id: str | None = None, canonical_recovery: Mapping[str, Any] | None = None, restart_regression: Mapping[str, Any] | None = None, readonly_audit: Mapping[str, Any] | None = None, page_wiring: Mapping[str, Any] | None = None, dashboard_refresh: Mapping[str, Any] | None = None, local_review: Mapping[str, Any] | None = None, operator_acknowledged: bool = False) -> dict[str, Any]:
    evidence = {
    ...
    report = {
        ...
        **summary,
        ...
        "safety_gates": {"live_mutation": FORBIDDEN, "model_training": FORBIDDEN, "stored_data_mutation": FORBIDDEN, "automatic_live_promotion": FORBIDDEN, "proof_overwrite": FORBIDDEN, "issue_close_requires_operator_ack": str(not operator_acknowledged)},
        "preview_only": True,
        "files_written": 0,
        "live_changes": 0,
...
def build_proof_hardening_closeout_from_text(workspace_id: str | None = None, canonical_recovery_json: str | None = None, restart_regression_json: str | None = None, readonly_audit_json: str | None = None, page_wiring_json: str | None = None, dashboard_refresh_json: str | None = None, local_review_json: str | None = None, operator_acknowledged: bool = False) -> dict[str, Any]:
    return build_proof_hardening_closeout(workspace_id, parse_json_object(canonical_recovery_json), parse_json_object(restart_regression_json), parse_json_object(readonly_audit_json), parse_json_object(page_wiring_json), parse_json_object(dashboard_refresh_json), parse_json_object(local_review_json), operator_acknowledged)
