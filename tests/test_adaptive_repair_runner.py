...
def test_runner_scans_uploaded_rows_and_preserves_tracker_baseline(tmp_path):
    ...
    assert report.activation_gate["checks"]["live_pick_changes_allowed"] is False
...
def test_runner_reports_missing_market_data_and_watchlists_remain_inactive():
    ...
    assert all(candidate["status"] == "shadow_watchlist" for candidate in report.pattern_candidates)
    ...
    assert all(candidate["production_activation"] is False for candidate in report.pattern_candidates)
